...
namespace cam
{
   typedef unsigned int CameraId; //!< 相机设备ID
   ...
   public:
       ...
       * @brief 从缓冲区中获取一帧图像
       * 
       * @attention 当缓冲区中没有图像的时候会报错，子类有必要需要覆盖该方法！
       * 
       * @return PackedFrame
       */
       ...
       * @brief 停止图像采集
       * 
       * @attention 调用此函数后，程序中相机无法再次采集图像！
       */
   ...
   protected:
       ...
       * @brief 采集一帧图像
       * 
       * @attention 底层使用了多线程实现，初始化的时候会启用图像读取线程
       * 
       * @return true 
       * @return false 
       */
   ...
   public:
       ...
       * @brief 更新参数组
       * 
       * @attention 每次调用都会读取底层配置文件，不建议频繁调用！
       * 
       * @param group_id 
       * @param file_path
       * @return true 
